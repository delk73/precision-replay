...
| `impl Mul for I64F64 { ... }` | **HLR-MATH-OPS-002** / **LLR-REPLAY-MATH-OPS-002** | Maps to raw sign-isolated magnitude scaling: the absolute product magnitude is shifted/truncated by 64 bits, then the isolated sign is reapplied. `SVCP-MATH-PRO-002a` provides active proof coverage for tiny fractional products truncating to zero, including negative and mixed-sign products; bounded full semantic equivalence remains pending under `SVCP-MATH-PRO-002b`, and full limb/cross-term correspondence remains pending under `SVCP-MATH-PRO-002c`. |
