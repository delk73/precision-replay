...
| **HLR-WITNESS-ENV** / **LLR-WITNESS-ENV** | Raw ADC envelope judgment boundary. | requirements-defined; implementation-deferred; verification-deferred. | Judgment applies only to admitted raw ADC observations in a declared context. The first judgment definition uses `raw_adc_min`, `raw_adc_max`, `min_sample_count`, and `allow_malformed_witness_lines`, with allowed results `pass`, `fail`, `inconclusive`, and `not_applicable`. When `allow_malformed_witness_lines=false`, malformed witness rows must not pass; when `true`, malformed witness rows remain outside admitted observations and may be tolerated only under the existing best-effort/non-lossless capture boundary. No envelope checker, calibrated measurement, UART losslessness, timing proof, signal quality, stimulus adequacy, hardware correctness, board qualification, release readiness, or certification posture is claimed. |
