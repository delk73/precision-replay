...
| `if cross_hi != 0 \|\| hh != 0` | **LLR-REPLAY-MATH-OPS-002** | Maps to Step 3 exponential overflow gating: if the upper blocks contain any bits, a saturation exception is guaranteed and must panic. |
| `impl Mul for I64F64 { ... }` | **HLR-MATH-OPS-002** / **LLR-REPLAY-MATH-OPS-002** | Maps to Step 2 raw truncation via arithmetic right-shifting by 64 bits (`ll >> 64`) to restore the scaling alignment default. |
...
| `if final_abs_bits > i128::MAX` | **LLR-REPLAY-MATH-OPS-002** | Maps to Step 3 capacity boundary gating, protecting signed limits while allowing the unique `i128::MIN` absolute match if `out_negative` is true. |
